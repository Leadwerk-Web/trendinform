...
const onScroll = () => {
  nav?.classList.toggle('is-scrolled', nav.classList.contains('legal-nav') || window.scrollY > 40);
...
document.querySelectorAll('.case[data-usecase]').forEach(card => {
  card.addEventListener('click', () => {
    ...
    const radio = document.querySelector(`#usecaseOptions input[value="${value}"]`) ||
      [...document.querySelectorAll('.wpforms-container input[type="radio"]')].find(input => input.value === value);
    if (radio) radio.checked = true;
  });
});

// ===== WPForms: portable Datenschutz-Link =====
document.querySelectorAll('.wpforms-field-gdpr-checkbox .wpforms-field-label-inline').forEach(label => {
  if (label.querySelector('a') || !window.leadwerkTheme?.privacyUrl) return;
  const text = label.textContent || '';
  const needle = 'Datenschutzerklärung';
  const offset = text.indexOf(needle);
  if (offset < 0) return;
  label.textContent = text.slice(0, offset);
  const link = document.createElement('a');
  link.href = window.leadwerkTheme.privacyUrl;
  link.textContent = needle;
  label.append(link, document.createTextNode(text.slice(offset + needle.length)));
});
...
if (form) {
  ...
  const backButton = document.getElementById('backStep1');
